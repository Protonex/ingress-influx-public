...
{
    const double tilesOnOneEdge = pow(2.0,zoomLevel);
    const quint16 tileSize = this->tileSize();
    double x = (ll.x()+180.0) * (tilesOnOneEdge*tileSize)/360.0; // coord to pixel!
    double y = (1.0-(log(tan(PI/4+(ll.y()*deg2rad)/2.0)) /PI)) /2.0  * (tilesOnOneEdge*tileSize);
...
QPointF OSMTileSource::qgs2ll(const QPointF &qgs, quint8 zoomLevel) const
{
    const double tilesOnOneEdge = pow(2.0,zoomLevel);
    const quint16 tileSize = this->tileSize();
    double longitude = (qgs.x()*(360/(tilesOnOneEdge*tileSize)))-180;
    double latitude = rad2deg*(atan(sinh((1-qgs.y()*(2/(tilesOnOneEdge*tileSize)))*PI)));
